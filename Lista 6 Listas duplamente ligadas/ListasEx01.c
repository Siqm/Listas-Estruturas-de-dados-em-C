...
void imprimeClassificacaoEscolhida (Lista *lista) {
	int pesquisa;
	...
	if(isEmpty(lista)){
		...
		printf("Classificações disponíveis: \n");
		printf("1-Terror \n2-Aventura \n3-Ficção \n4-Ciencia \n");
		printf("Insira o numero da calssificação: ");
		scanf("%i", &pesquisa);
		...
		switch(pesquisa) {
			...
			case 1:
				printf("Existem %i livros de Terror", lista->contaTerror);
			break;
			
			case 2:
				printf("Existem %i livros de Aventura", lista->contaAvent);
			break;
			
			case 3:
				printf("Existem %i livros de Ficção", lista->contaFic);
			break;
			
			case 4:
				printf("Existem %i livros de Ciência", lista->contaCienc);
			break;
		}
	}
}
...
void menorPreco (Lista *lista) {
	Livro *ptr = lista->inicio;
	Livro *menor = lista->inicio;
	...
	if (lista->inicio == NULL) {
		...
		while(ptr != NULL) {
			if (menor->preco < ptr->preco) {
				menor->preco = ptr->preco;
			}
			
			ptr = ptr->proximo;
		}
		printf("O menor valor é %.2f", menor->preco);
	}
}
	...
	int opcao = -1;
		...
		while (opcao != 99) {	
			...
			switch(opcao) {
				...
				case 14:
					imprimeClassificacaoEscolhida(novaLista);
				break;
					
				case 15:
					imprimeClassificacao (novaLista);
				break;
